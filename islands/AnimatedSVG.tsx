...
export default function AnimatedSVG() {
  useEffect(() => {
    console.log('in eff')
    // TypeScript for dynamic behaviors
    const stripes = document.querySelectorAll<SVGRectElement>(".stripe");

    function updateStripeSpeeds() {
      stripes.forEach((stripe, index) => {
        const speed = Math.random() * 4 + 3; // Randomize animation duration between 3-7 seconds
        stripe.style.animationDuration = `${speed}s, 10s`;
      });
    }

    function updateStripeColors() {
      stripes.forEach((stripe) => {
        const randomHue = Math.floor(Math.random() * 360); // Random hue
        stripe.style.fill = `hsl(${randomHue}, 70%, 50%)`;
      });
    }
